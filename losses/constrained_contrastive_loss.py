...
class lossObj:
    def __init__(self,cfg):
        ...
        self.use_mask_sampling     = True#cfg.use_mask_sampling
        self.epsilon               =  1e-7
    ...
    def calc_CCL_batchwise_double(self, y_true, y_pred):
        loss_cluster = self.calc_CCL_batchwise(y_true[0], y_pred)
        loss_mind = self.calc_CCL_batchwise(y_true[1], y_pred)
        return loss_cluster + loss_mind
...
class lossObj3D:
    def __init__(self, cfg):
        ...
        self.partial_decoder       = cfg.partial_decoder
        self.contrastive_loss_type = cfg.contrastive_loss_type
        self.use_mask_sampling     = True  # Assuming it's enabled for 3D
        self.epsilon               = 1e-7
    ...
    def calc_CCL_imagewise(self, ft_img, cluster_arr):
        ...
        if patch_size == 1:
            ...
            pred_blk = tf.extract_volume_patches(ft_img,
                                                ksizes=[1, patch_size, patch_size, patch_size, 1],
                                                strides=[1, patch_size, patch_size, patch_size, 1],
                                                padding='VALID')
            # pred_blk = extract_3d_patches(ft_img,patch_size)
            #print(pred_blk.shape)
            pred_blk = tf.squeeze(pred_blk)

        num_patches, num_patches_z = pred_blk.shape[0], pred_blk.shape[2]

        memory_bank = tf.reshape(pred_blk, (num_patches * num_patches * num_patches_z, -1))
        cluster_arr = tf.reshape(tf.squeeze(cluster_arr), (num_patches * num_patches * num_patches_z, -1))
    ...
    def sample_input_patches(self, num_patches, mask=None, exclude_num=3):
        ...
        if mask is not None:
            ...
            img_indices = np.reshape(np.arange(0, num_patches * num_patches * num_patches),
                                     (num_patches, num_patches, num_patches))
            img_indices_cropped = img_indices[exclude_num:-exclude_num, exclude_num:-exclude_num, exclude_num:-exclude_num]
            img_indices_flat = np.reshape(img_indices_cropped, (-1,))
            random_ip_idx = np.random.choice(img_indices_flat, num_samples_loss_eval, replace=False)
        return random_ip_idx
...
def extract_3d_patches(input_tensor, patch_size):
    """
    Extracts 3D patches using conv3d.

    Args:
        input_tensor: A 5D tensor of shape [batch, depth, height, width, channels].
        patch_size: Integer, the size of the cube patch (assumes cubic patches).

    Returns:
        Tensor containing extracted patches.
    """
    batch, depth, height, width, channels = input_tensor.shape

    # Define a 3D kernel of shape [patch_size, patch_size, patch_size, channels, channels]
    kernel = tf.eye(num_rows=channels, batch_shape=[patch_size, patch_size, patch_size])  # Identity kernel
    kernel = tf.reshape(kernel, [patch_size, patch_size, patch_size, channels, channels])

    # Apply 3D convolution to extract patches
    patches = tf.nn.conv3d(
        input=input_tensor,
        filters=kernel,
        strides=[1, patch_size, patch_size, patch_size, 1],  # Stride is same as patch size
        padding="VALID"
    )
