...
import configs.registration_ft as cfg
import configs.finetune as cfg_finetune
...
from losses.reg_losses import  design_loss, smooth, residuce_loss, dice_loss,none_loss, AME
...
def main(debug):
    ...
    # non-rigid
    customLoss = [design_loss(parameter_mi=cfg.para_mi, parameter=cfg.para_jl).mi_clipmse, AME().loss, dice_loss().loss, smooth().loss, residuce_loss]
    loss_weights = [1, 1, 0.01, 0.04, cfg.para_rl]

    AdamOpt = optimizers.Adam(learning_rate=cfg.lr_pretrain, clipvalue=1.0)

    mm_utils = modelObj(cfg)
    ae_reg = mm_utils.reg_unet_ft(cfg_finetune, cfg.weighted)
