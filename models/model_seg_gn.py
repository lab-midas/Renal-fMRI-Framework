...
class modelObj:
    ...
    def encoder_decoder_network(self, num_dec_levels=5, enc_pretr_wts=None, 
                                enc_freeze=False, add_PH=False, name='dec_model', PH_str = '', two_out = False):
        ...
        if add_PH:  
            ...
            if two_out:
                enc_dec = Model(inputs=[inputs], outputs=[PH_B, PH_B], name=name)
            else:
    ...
    def encoder_decoder_segmentation(self, num_dec_levels=5, enc_pretr_wts=None,
                                enc_freeze=False, name='seg_model', num_classes=3):
        ...
        layer_idx = len(no_filters) - 1
        tmp_dec = self.decoder_block_expand(enc_c6, no_filters[layer_idx], enc_layers_list[layer_idx - 1],
                                            block_name=layer_idx)
        for dec_layer in range(1, num_dec_levels):
            print('Generating dec layer ', dec_layer)
            layer_idx = layer_idx - 1
            tmp_dec = self.decoder_block_expand(tmp_dec, no_filters[layer_idx], enc_layers_list[layer_idx - 1],
                                                block_name=layer_idx)

        model_op = Conv2D(16, 3, padding='same', kernel_initializer=self.kernel_init)(tmp_dec)
        model_op = GroupNormalization(groups=4)(model_op)
        seg_op2 = Activation('relu')(model_op)
        seg_op = Conv2D(num_classes, 1, name='seg_layer', padding='same', use_bias=False,
