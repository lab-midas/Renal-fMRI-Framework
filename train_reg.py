...
import configs.registration as cfg
...
from losses.reg_losses import Grad, MutualInformation, mse_from_difference_loss, Dice, mi, design_loss, smooth, residuce_loss, dice_loss,none_loss, PCA_loss
...
def main(debug):
    ...
    # non-rigid
    #customLoss = [design_loss(parameter_mi=cfg.para_mi, parameter=cfg.para_jl).mi_clipmse, none_loss, smooth().loss, none_loss]
    customLoss = [PCA_loss, none_loss, smooth().loss, none_loss]
    loss_weights = [1, 0.25, 1, cfg.para_rl]

    AdamOpt = optimizers.Adam(learning_rate=cfg.lr_pretrain, clipvalue=1.0)

    mm_utils = modelObj(cfg)
    ae_reg = mm_utils.reg_unet(cfg.weighted)
